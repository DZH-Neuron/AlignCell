...
class FastAttention(nn.Module):
    def __init__(self, dim_heads, nb_features = None, ortho_scaling = 0, causal = False, output_attentions = False, kernel_fn = nn.ReLU(), no_projection = False, generalized_attention=False):
        super().__init__()
    ...
    def forward(self, q, k, v, output_attentions = False):
        ...
        if output_attentions:
            ...
            attn_weights = torch.zeros(1, q.shape[0], q.shape[2], q.shape[2]).to(device).to(torch.float16)
