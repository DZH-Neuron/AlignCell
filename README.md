...
After downloading, switch to the first AlignCell directory, first run  ```sh pip install -r requirements.txt``` to install the dependencies, and then run ```sh pip install -e .``` to complete the installation.
